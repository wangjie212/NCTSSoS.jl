struct NCStateWord{V,M}
    ...
    function NCStateWord(monos::Vector{Monomial{V,M}}, nc_word::Monomial{V,M}) where {V,M}
        ...
        return new{V,M}(isempty(monos) ? [one(Monomial{V,M})] : monos, nc_word)
    end
end
...
function Base.:(*)(a::NCStateWord{V,M}, b::Monomial{V,M}) where {V,M}
    return NCStateWord(a.sw, a.nc_word * b)
end
...
function Base.one(::Type{NCStateWord{V,M}}) where {V,M}
    return NCStateWord([one(Monomial{V,M})], one(Monomial{V,M}))
end
...
function Base.show(io::IO, ncsw::NCStateWord)
    return print(
        io,
        join(["<$(string(mono))>" for mono in ncsw.sw], " * ") *
        " * " *
        string(ncsw.nc_word),
    )
end
...
function Base.:(==)(a::NCStateTerm, b::NCStateTerm)
    return isequal(a.coef, b.coef) && (a.ncstate_word == b.ncstate_word)
end
Base.:(+)(a::NCStateTerm, b::NCStateTerm) = NCStatePolynomial([a, b])
function Base.:(-)(a::NCStateTerm{V,M,T}, b::NCStateTerm{V,M,T}) where {V,M,T}
    return NCStatePolynomial([a, -b])
end
...
function Base.:(*)(a::NCStateTerm{V,M,T}, b::Monomial{V,M}) where {V,M,T}
    return NCStateTerm(a.coef, a.ncstate_word * b)
end
function Base.:(*)(a::NCStateTerm{V,M,T}, b::NCStateTerm{V,M,T}) where {V,M,T}
    return NCStateTerm(a.coef * b.coef, a.ncstate_word * b.ncstate_word)
end
Base.hash(a::NCStateTerm) = hash((hash(a.coef), hash(a.ncstate_word)))
function Base.one(::Type{NCStateTerm{V,M,T}}) where {V,M,T}
    return NCStateTerm(one(T), one(NCStateWord{V,M}))
end
function Base.zero(::Type{NCStateTerm{V,M,T}}) where {V,M,T}
    return NCStateTerm(zero(T), one(NCStateWord{V,M}))
end
...
function Base.show(io::IO, ncst::NCStateTerm)
    ...
    return print(io, " * " * string(ncst.ncstate_word))
end
...
function Base.show(io::IO, ncsp::NCStatePolynomial)
...
function Base.:(*)(a::NCStatePolynomial, b::NCStatePolynomial)
    return NCStatePolynomial(
        vec(map(x -> x[1] * x[2], product(a.nc_state_terms, b.nc_state_terms)))
    )
end
function Base.:(*)(n, a::NCStatePolynomial{V,M,T}) where {V,M,T}
    return NCStatePolynomial(T(n) .* a.nc_state_terms)
end
function Base.:(+)(a::NCStatePolynomial, b::NCStatePolynomial)
    return NCStatePolynomial([a.nc_state_terms; b.nc_state_terms])
end
Base.:(+)(a::NCStatePolynomial, b::NCStateTerm) = NCStatePolynomial([a.nc_state_terms; b])
function Base.:(-)(a::NCStatePolynomial{V,M,T}, b::NCStatePolynomial{V,M,T}) where {V,M,T}
    return NCStatePolynomial([a.nc_state_terms; -one(T) .* b.nc_state_terms])
end
function Base.:(-)(a::NCStatePolynomial{V,M,T}, b::NCStateTerm) where {V,M,T}
    return NCStatePolynomial([a.nc_state_terms; -one(T) * b])
end

function Base.one(::NCStatePolynomial{V,M,T}) where {V,M,T}
    return NCStatePolynomial([one(NCStateTerm{V,M,T})])
end
function Base.zero(::NCStatePolynomial{V,M,T}) where {V,M,T}
    return NCStatePolynomial([zero(NCStateTerm{V,M,T})])
end
function Base.zero(::Type{NCStatePolynomial{V,M,T}}) where {V,M,T}
    return NCStatePolynomial([zero(NCStateTerm{V,M,T})])
end
function DynamicPolynomials.variables(ncsp::NCStatePolynomial)
    return sorted_union(variables.(ncsp.nc_state_terms)...)
end
function DynamicPolynomials.degree(ncsp::NCStatePolynomial)
...
function get_state_basis(variables::Vector{Variable{V,M}}, d::Int, reducer) where {V,M}
    return map(
        a -> NCStateWord(a[1], a[2]),
        mapreduce(vcat, 0:d) do nc_deg
            nc_basis = unique(
                filter(m -> degree(m) == nc_deg, reducer.(monomials(variables, nc_deg)))
            )
            cw_deg = d - nc_deg
            cw_basis = unique!([
                begin
    ...
    eval(
        quote
            ...
            function $(symb)(ncst::NCStateTerm)
                return NCStateTerm($(take_adj)(ncst.coef), $(symb)(ncst.ncstate_word))
            end

            function $(symb)(spo::NCStatePolynomial)
                return NCStatePolynomial($(symb).(spo.nc_state_terms))
            end
        end,
    )
end
