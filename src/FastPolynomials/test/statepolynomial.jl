...
@testset "NCTraceStatePolynomial Components" begin
    ...
    @testset "NCTraceStatePolynomial" begin
        sws = NCStateWord.(Ref(MaxEntangled),[[x[1] * x[2], x[2]^2], [x[1] * x[2]], [x[2]^3]], Ref(one(x[1])))
        sp = ncstatepoly([1.0, 2.0, 5.0], sws)
        @test string(sp) ==
            "2.0 * tr(x₁¹x₂¹) * 1 + 5.0 * tr(x₂³) * 1 + 1.0 * tr(x₁¹x₂¹) * tr(x₂²) * 1"
        sws_rep = NCStateWord.(Ref(MaxEntangled),
        ...
        sws_diff = NCStateWord.(Ref(MaxEntangled),
        ...
        @test sort(monomials(sp)) == sort(
            NCStateWord.(Ref(MaxEntangled),[[x[2]^2, x[1] * x[2]], [x[1] * x[2]], [x[2]^3]], Ref(one(x[1])))
        )
...
@testset "Trace State Polynomial" begin
    ...
    @testset "Trace State Polynomial Arithmetic" begin
        ...
        sps = [
            1.0 * tr(x[1] * x[2]) + 2.0 * tr(x[2] * x[1]), 2.0 * tr(x[2]^2) + 3.0 * tr(x[1]^2)
        ]
    ...
    @testset "variables of NC Trace State Polynomial" begin
        ...
        sp = 1.0 * (tr(x[4] * x[5]) * tr(x[6] * x[7])) + 2.0 * tr(x[8]) + 3.0 * tr(x[9] * x[10])
    ...
    @testset "Trace State Polynomial Op comparison" begin
        ...
        state_word_order1 = NCStateWord.(Ref(MaxEntangled),[[a^2], [b^2], [c^2]], Ref(one(a)))
        state_word_order2 = NCStateWord.(Ref(MaxEntangled),[[b^2], [c^2], [a^2]], Ref(one(a)))
...
@testset "NCStatePolynomial Components" begin
    ...
    @testset "NCStatePolynomial" begin
        sws = NCStateWord.(Ref(Arbitrary),[[x[1] * x[2], x[2]^2], [x[1] * x[2]], [x[2]^3]], Ref(one(x[1])))
        sp = ncstatepoly([1.0, 2.0, 5.0], sws)
        @test string(sp) ==
            "2.0 * <x₁¹x₂¹> * 1 + 5.0 * <x₂³> * 1 + 1.0 * <x₁¹x₂¹> * <x₂²> * 1"
        sws_rep = NCStateWord.(Ref(Arbitrary),
            [[x[1] * x[2], x[2]^2], [x[1] * x[2]], [x[2]^2, x[1] * x[2]], [x[2]^3]],
            Ref(one(x[1])),
        )
        sp_rep = ncstatepoly([0.5, 2.0, 0.5, 5.0], sws_rep)
        ...
        sws_diff = NCStateWord.(Ref(Arbitrary),
            [[x[1] * x[2], x[2]^2], [x[1] * x[2]], [x[2]^3, x[1]]], Ref(one(x[1]))
        )
        sp_diff = ncstatepoly([1.0, 2.0, 5.0], sws_diff)
        @test sp_diff != sp
        ...
        @test sort(monomials(sp)) == sort(
            NCStateWord.(Ref(Arbitrary),[[x[2]^2, x[1] * x[2]], [x[1] * x[2]], [x[2]^3]], Ref(one(x[1])))
        )
