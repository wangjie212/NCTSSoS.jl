...
@testset "NCStatePolynomial Components" begin
    ...
    @testset "NCStateWord" begin
        ...
        @test sort(total_basis) == sort(map(x->NCStateWord(x[1],x[2]),zip(c_words,nc_words)))
    end
    ...
    @testset "NCStatePolynomial" begin
        ...
        @test string(sp) == "2.0 * <x[1]¹x[2]¹> * 1 + 5.0 * <x[2]³> * 1 + 1.0 * <x[1]¹x[2]¹> * <x[2]²> * 1"
        ...
        @test sort(monomials(sp)) == sort(NCStateWord.([[x[2]^2, x[1] * x[2]], [x[1] * x[2]], [x[2]^3]], Ref(one(x[1]))))
...
@testset "State Polynomial" begin
    @testset "free state polynomial" begin
        ...
        @test string(spop) == "2.0 * <x[1]¹x[2]¹> + 3.0 * <x[2]³> + 1.0 * <x[1]¹x[2]¹> * <x[2]²> + 0.5 * <x[1]²> * <x[2]¹x[1]¹>"
    end
    ...
    @testset "nc state polynomial" begin
        ...
        @test string(spop) == "3.0 * <x[2]³> * 1 + 2.0 * <x[1]¹x[2]¹> * x[2]² + 1.0 * <x[1]¹x[2]¹> * <x[2]²> * x[1]¹"

        @test degree(spop) == 5
        @test sort(monomials(spop)) == sort(map(a -> NCStateWord(a[1], Monomial(a[2])), [([x[2]^3], one(x[1])), ([x[2]^2, x[1] * x[2]], x[1]), ([x[1] * x[2]], x[2]^2)]))
    end

    @testset "State Polynomial Arithmetic" begin
        @ncpolyvar x[1:2] 

        sps = [1.0 * ς(x[1]*x[2])+ 2.0 * ς(x[2]*x[1]), 2.0 * ς(x[2]^2) + 3.0 * ς(x[1]^2)]

        sp_prod = 2.0 * ς(x[1]*x[2]) * ς(x[2]^2) + 4.0 * ς(x[2]*x[1]) * ς(x[2]^2) + 3.0 * ς(x[1]*x[2]) * ς(x[1]^2) + 6.0 * ς(x[2]*x[1]) * ς(x[1]^2)
        @test sps[1] * sps[2] == sp_prod
        sp_sum = 1.0 * ς(x[1]*x[2]) + 2.0 * ς(x[2]*x[1]) + 2.0 * ς(x[2]^2) + 3.0 * ς(x[1]^2)
