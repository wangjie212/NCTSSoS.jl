...
function StatePolyOpt(objective::StatePolynomial{V,M,T}; constraints=Any[], is_equality=fill(false, length(constraints)), comm_gp=Variable{V,M}[], is_unipotent::Bool=false, is_projective::Bool=false) where {V,M,T}
    ...
    @assert issubset(comm_gp, vars) "The commutative variables must be a subset of the variables."
    @assert !(is_unipotent && is_projective) "The problem cannot be both unipotent and projective."
    return StatePolyOpt{V,M,T}(objective, cons, is_eq, vars, Set(comm_gp), is_unipotent, is_projective)
end
